...
namespace SZ2.WebSocketGaugeServer.WebSocketServer.SSMWebSocketServer
{
    public class Startup
    {
        ...
        {
            services.AddSingleton<SSMCOMService>(_ => new SSMCOMService("/dev/ttyUSB1"));
        }
        ...
        private async Task processReceivedMessage(WebSocket ws, SSMCOMService service, SSMCOMWebsocketSessionParam sessionParam, IPAddress destAddress)
        {
            // Get mode code
            try
            {
