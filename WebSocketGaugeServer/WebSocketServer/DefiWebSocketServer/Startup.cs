...
namespace SZ2.WebSocketGaugeServer.WebSocketServer.DefiWebSocketServer
{
    public class Startup
    {
        ...
        private async Task processReceivedMessage(WebSocket ws, DefiCOMWebsocketSessionParam sessionParam, IPAddress destAddress, ILogger logger, CancellationToken ct)
        {
            // Get mode code
            try
            {
                ...
                if (wsmessage.MessageType == WebSocketMessageType.Text)
                {
                    ...
                    switch (receivedJSONmode)
                    {
                        case (ResetJSONFormat.ModeCode):
                            sessionParam.reset();
                            await send_response_msg(ws, "Defi Websocket all parameter reset.", destAddress, logger, ct);
                            break;
                        case (DefiWSSendJSONFormat.ModeCode):
                            ...
                            await send_response_msg(ws, "Defi Websocket send_flag for : " + msg_obj_wssend.code.ToString() + " set to : " + msg_obj_wssend.flag.ToString(), destAddress, logger, ct);
                            break;
                        ...
                        case (DefiWSIntervalJSONFormat.ModeCode):
                            ...
                            await send_response_msg(ws, "Defi Websocket send_interval to : " + msg_obj_interval.interval.ToString(), destAddress, logger, ct);
                            break;
                        default:
