...
//using DefiSSMCOM.Communication;
            ...
            public class DefiCOM
            {
				...
				public string PortName
                {
                    ...
                    set
                    {
                        ...
                        {
							DefiSSMCOM.Communication.Alert.message(ex1.Message, "DefiCOMのエラー");
                        }
                    }
                }
                ...
                private void communicate_realtime()
                {
                    ...
                    {
						DefiSSMCOM.Communication.Alert.message(ex.Message,"DefiCOMポートが開けません");
                    }
                    catch (System.InvalidOperationException ex)
                    {
						DefiSSMCOM.Communication.Alert.message(ex.Message,"DefiCOMポートはすでに開かれています。");
                    }
                    catch (System.UnauthorizedAccessException ex)
                    {
						DefiSSMCOM.Communication.Alert.message(ex.Message,"DefiCOMポートへのアクセスを拒否されました。");
                    }
                    finally
                    {
                        _communicate_realtime_start = false;
